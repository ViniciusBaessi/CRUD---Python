...
def principal(page: ft.Page):
    ...
    class Input_dados(ft.TextField):
        def __init__(self, label, hint_text):
            super().__init__()
            self.label = label  # Agora você passa a label diretamente ao instanciar
            self.label_style = ft.TextStyle(color="#8a8a8a")
            self.border_color = "Orange"
            self.hint_text = hint_text  # Mantém o placeholder como um parâmetro opcional
    ...
    Menu1 = ft.Container(
        content=ft.Column(
            controls=[
                Input_dados(label="Nome", hint_text="Responsável pela reserva"),
                Input_dados(label="Quantidade de pessoas", hint_text="De 0 a 10"),
                Input_dados(label="Data", hint_text="DD/MM/AAAA"),
                Input_dados(label="Horário", hint_text="10h-AM até 11h-PM")
            ]
        )
    )
